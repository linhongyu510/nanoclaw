...
describe('provider contract verifier', () => {
  ...
  it('uses the pinned Bun fallback and compares every contract inventory', async () => {
    ...
    expect(commands).toContain(
      'pnpm exec vitest run src/provider-contracts src/providers setup/provider-contracts setup/provider-contract.test.ts setup/providers',
    );
  ...
  it('runs the OpenCode CLI pin guard when the skill installed it', async () => {
    ...
    const result = await verifyProviderContracts(root, {
      ...
      exec: (command) => {
        commands.push(command);
        if (command.includes('provider-contract-names.ts')) {
          return JSON.stringify({ host: ['claude'], setup: ['claude'], hostProviders: [], setupProviders: ['claude'] });
        }
        if (command.includes('src/provider-contracts/names.ts')) {
          return JSON.stringify({ contracts: ['claude'], providers: ['claude'] });
        }
    ...
    expect(result.status).toBe('passed');
    expect(commands).toContain(
      'pnpm exec vitest run src/provider-contracts src/providers setup/provider-contracts setup/provider-contract.test.ts setup/providers src/opencode-cli-tools.test.ts',
    );
  });
