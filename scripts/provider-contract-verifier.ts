...
export async function verifyProviderContracts(
  ...
  try {
    ...
    await run(
      ...
      `pnpm exec vitest run src/provider-contracts src/providers setup/provider-contracts setup/provider-contract.test.ts setup/providers${optionalHostTests}`,
    );
    ...
    if (JSON.stringify(host.setup) !== expected || JSON.stringify(runtime.contracts) !== expected) {
      throw new Error(
        `Provider contract inventories differ: host=${expected}, setup=${JSON.stringify(host.setup)}, runtime=${JSON.stringify(runtime.contracts)}`,
      );
    }
